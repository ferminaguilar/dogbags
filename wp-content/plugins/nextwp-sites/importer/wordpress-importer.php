...
if ( class_exists( 'WP_Importer' ) ) {
	...
	<?php endif; ?>
		...
		<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'cubewp-framework' ); ?>" /></p>
	</form>
	<?php
		}
		...
		function process_wc_products() {
			$this->posts = apply_filters( 'wp_import_posts', $this->posts );
			foreach ( $this->posts as $post ) {
				apply_filters( 'nwp_import_product_attributes', $post );
			}
		}
					...
					foreach ( $post['postmeta'] as $meta ) {
						...
						if ( $key ) {
							...
							if (function_exists('wp_slash_strings_only')){
								...
								if (function_exists('addslashes') && (strpos( $key, 'lp' ) || strpos( $key, 'cubewp-framework' )) ){
		...
		// 			__( 'Fetching attachments is not enabled', 'cubewp-framework' ) );
		...
		// 	else
		// 		return new WP_Error( 'attachment_processing_error', __('Invalid file type', 'cubewp-framework') );
		...
		// function process_attachment( $post, $url ) {
		// 	// Check if the URL is absolute, and if not, prepend the base URL
		// 	if ( preg_match( '|^/[\w\W]+$|', $url ) ) {
		// 		$url = rtrim( $this->base_url, '/' ) . $url;
		// 	}
		
		// 	// Mimic file type detection based on the URL
		// 	$filetype = wp_check_filetype( basename( $url ) );
		// 	if ( empty( $filetype['type'] ) ) {
		// 		return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'cubewp-framework' ) );
		// 	}
		
		// 	// Set the MIME type and GUID for the attachment post
		// 	$post['post_mime_type'] = $filetype['type'];
		// 	$post['guid'] = $url; // Use the URL directly without fetching
		
		// 	// Insert the attachment post into the database
		// 	$post_id = wp_insert_attachment( $post, null ); // Set file path to null since we are not fetching the file
		
		// 	// Skip generating metadata, but you can set default metadata if needed
		// 	wp_update_attachment_metadata( $post_id, [
		// 		'width' => 0,
		// 		'height' => 0,
		// 		'file' => '', // Empty since no local file exists
		// 	]);
		...
		function process_attachment( $post, $url ) {
			...
			// Replace the remote domain with the local uploads base URL
			$url = $upload_dir['url'] . '/' . $filename;
		
			// Construct the local file path using the `path` from wp_upload_dir
			$file_path = trailingslashit( $upload_dir['path'] ) . $filename;
		
			// Ensure the file exists in the local uploads folder
			if ( ! file_exists( $file_path ) ) {
				return new WP_Error( 'attachment_processing_error', __( 'File does not exist in uploads folder', 'cubewp-framework' ) );
			}
		
			// Get the file type and ensure it's a valid image
			$filetype = wp_check_filetype( basename( $file_path ) );
			if ( empty( $filetype['type'] ) ) {
				return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'cubewp-framework' ) );
			}
		
			// Set up attachment post data
			$post['post_mime_type'] = $filetype['type'];
			$post['guid'] = $url; // Use the original URL as the GUID
			...
			// Skip the metadata generation if you don't need to regenerate thumbnails
			$metadata = wp_get_attachment_metadata( $post_id );
		
			// If metadata doesn't exist or is incomplete, generate it
			if ( empty( $metadata ) ) {
				// Only generate metadata for the original image (not thumbnails)
				//$metadata = wp_generate_attachment_metadata( $post_id, $file_path );
			}
		
			// Update the metadata (avoiding any slow thumbnail regeneration)
