...
if (!function_exists('value_pack_inject_heading_controls')) {
    function value_pack_inject_heading_controls($element, $args)
    {

        $element->start_controls_section(
            'image_logo_section',
            [
                'label' => __('Logo Options', 'valuepack-addons'),
        ...
        $element->add_control(
            'enable_logo',
            [
                'label' => esc_html__('Enable Logo', 'valuepack-addons'),
        ...
        $element->add_control(
            'logo_behavior',
            [
                'label' => esc_html__('Logo Behavior', 'valuepack-addons'),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'default' => esc_html__('Default', 'valuepack-addons'),
                    'hover' => esc_html__('Hover Effect', 'valuepack-addons'),
                ],
                'default' => 'default',
                'condition' => [
                    'enable_logo' => 'yes',
    ...
    add_action('elementor/element/image/section_image/after_section_end', 'value_pack_inject_heading_controls', 10, 2);
}
 
if (!function_exists('value_pack_render_image_on_element')) {
    function value_pack_render_image_on_element($widget)
    {
        ...
        if (!empty($settings['enable_logo']) && $settings['enable_logo'] === 'yes') {
            $logo_behavior_class = !empty($settings['logo_behavior']) && $settings['logo_behavior'] === 'hover'
                ? 'logo-hover'
                : 'logo-default';

            $widget->add_render_attribute('_wrapper', 'class', 'custom-logo ' . $logo_behavior_class);
        } 
    ...
    add_action('elementor/frontend/before_render', 'value_pack_render_image_on_element', 1);
}
