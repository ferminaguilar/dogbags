<?php
class CubeWp_Post_Types_List_Table extends WP_List_Table{
    
    public static $posttypes = array();
    function __construct(){
    global $status, $page;

        parent::__construct(self::$posttypes );
    ...
    public function Post_types(){
        ...
        if (isset($get_CustomTypes) && !empty($get_CustomTypes)) {
            foreach ($get_CustomTypes as $single_cpt) {
                $data = array();
                $data['singular']    = $single_cpt['label'];
                $data['plural']  = $single_cpt['singular'];
                $data['ajax']           = 'true';
            }
            self::$posttypes = $data;
        }
    ...
    */
    function column_plural_name( $item ) {
        $title = sprintf( '<a href="%s"><strong>' . $item['plural_name'] . '</strong></a>', CubeWp_Submenu::_page_action('cubewp-post-types','edit', '&postTypeid='.$item['slug'], '&_wpnonce='.wp_create_nonce( 'cwp_edit_post_type' )));
        $actions = [
            'edit' => sprintf( '<a href="%s">'. esc_html__('Edit', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('cubewp-post-types','edit', '&postTypeid='.$item['slug'], '&_wpnonce='.wp_create_nonce( 'cwp_edit_post_type' ))),
            
            'delete' => sprintf( '<a href="%s">'. esc_html__('Delete', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('cubewp-post-types','delete', '&postTypeslug='.$item['slug'], '&_wpnonce='.wp_create_nonce( 'cwp_delete_post_type' ))),
        ];
	...
	protected function column_cb( $item ) {
		return sprintf(
			...
			'cwp_types_bulk_action',  // Let's simply repurpose the table's singular label ("movie").
    ...
    protected function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) { 
            ...
            if(wp_verify_nonce( $nonce, 'bulk-' . $this->_args['plural'] ) ) {
                ...
                $bulk_request = isset($_REQUEST['cwp_types_bulk_action']) ? CubeWp_Sanitize_text_Array($_REQUEST['cwp_types_bulk_action']) : array(); //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
                foreach($bulk_request as $type){  
                    if($type){
                        unset($get_CustomTypes[$type]);
   ...
   public function prepare_items() {
        ...
        if (isset($get_CustomTypes) && !empty($get_CustomTypes)) {
            ...
            foreach ($get_CustomTypes as $single_cpt) {
                $data = array();
                $data['plural_name']    = $single_cpt['label'];
                $data['singular_name']  = $single_cpt['singular'];
                $data['slug']           = $single_cpt['slug'];
                $taxonomies             = get_object_taxonomies( $data['slug'], 'objects' );
