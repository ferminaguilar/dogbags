...
class CubeWp_settings_Custom_Fields_Table extends WP_List_Table{
    ...
    */
    function column_group_name( $item ) {
        ...
        $title = sprintf( '<a href="%s"><strong>' . $item['group_name'] .'</strong></a>'.$group_status, CubeWp_Submenu::_page_action('settings-custom-fields','edit', '&groupid='.absint( $item['ID']), '&_wpnonce='.wp_create_nonce( 'cwp_edit_group' )));
        $actions = [
            'edit' => sprintf( '<a href="%s">'. esc_html__('Edit', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('settings-custom-fields','edit', '&groupid='.absint( $item['ID']), '&_wpnonce='.wp_create_nonce( 'cwp_edit_group' ))),
            'duplicate' => sprintf( '<a href="%s">'. esc_html__('Duplicate', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('settings-custom-fields','duplicate', '&groupid='.absint( $item['ID']), '&_wpnonce='.wp_create_nonce( 'cwp_duplicate_group' ))),
        ];
        if( ! cubewp_custom_field_group_secure( absint( $item['ID'] ) ) ) {
            $actions['delete'] = sprintf( '<a href="%s">'. esc_html__('Delete', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('settings-custom-fields','delete', '&groupid='.absint( $item['ID']), '&_wpnonce='.wp_create_nonce( 'cwp_delete_group' )));
        }
        if(!empty($group_status)){
            $actions['activate'] = sprintf( '<a href="%s">'. esc_html__('activate', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('settings-custom-fields','activate', '&groupid='.absint( $item['ID']), '&_wpnonce='.wp_create_nonce( 'cwp_status_group' )));
        }else{
            $actions['deactivate'] = sprintf( '<a href="%s">'. esc_html__('deactivate', 'cubewp-framework') .'</a>', CubeWp_Submenu::_page_action('settings-custom-fields','deactivate', '&groupid='.absint( $item['ID']), '&_wpnonce='.wp_create_nonce( 'cwp_status_group' )));
        }
                        ...
                        if(isset($group_fields) && !empty($group_fields)){
                            ...
                            foreach($group_fields as $field){
                                if(isset($fieldOptions[$field])){
                                    unset($fieldOptions[$field]);
                                }
                            }
                            CWP()->update_custom_fields( 'settings', $fieldOptions );
                        }
    ...
    public static function set_option($name, $val) {
        if ($name) {
            $options = CWP()->get_custom_fields( 'settings' );
            $options = $options == '' ? array() : $options;
            $options[$name] = $val;
            return CWP()->update_custom_fields( 'settings', $options );
        } else {
            return false;
        }
    }
