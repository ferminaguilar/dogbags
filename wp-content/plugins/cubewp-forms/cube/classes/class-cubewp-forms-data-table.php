		...
		if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete') {
			...
			if (isset($_REQUEST['_wpnonce'])) {
				...
				if (wp_verify_nonce($nonce, 'cwp_delete_group')) {
					...
					wp_safe_redirect(CubeWp_Submenu::_page_action('cubewp-custom-form-data')); 
					exit();
				}
			}
		}
	...
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if (isset($form_data) && !empty($form_data) && !isset($formId)) {
			foreach ($form_data as $lead) {
				...
				$data['date_time'] = isset($lead['dete_time']) ? gmdate("m/d/Y h:i:s A T", $lead['dete_time']) : '';
				$_data[] = $data;
			}
			$data = $_data;
...
?>
				<div class="alignleft actions">
					...
					<select name="filter_form" id="filter-by-form">
						...
						?>
							<option <?php echo esc_attr($selected); ?> value="<?php echo esc_attr($form); ?>"><?php

																												// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
																												echo get_post_field('post_title', $form); ?></option>
