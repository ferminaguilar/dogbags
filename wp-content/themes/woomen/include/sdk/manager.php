...
function cwp_license_verification_callback()
{
	...
	if (isset($_POST['security_nonce']) && wp_verify_nonce($_POST['security_nonce'], 'cubewp-admin-nonce')) {
		...
		$api_url     = 'https://zemowp.com'; //route of api call which is cubewp website.
		$request_url = $api_url . '/wp-json/zemowp/v1/license/verify';
		$args        = array(
			'method'  => "POST",
			'timeout' => 100,
		...
		if (is_wp_error($response)) {
			$response = array(
				'status' => 'error',
				'msg'    => esc_html__('There is error with url or parameters', 'woomen')
			);
		...
		// Remove "woomen" from licenses_data
		if (isset($body->licenses_data) && ! empty($body->licenses_data)) {
			foreach ($body->licenses_data as $licenseKey => $licenseItems) {
				if (isset($licenseItems->woomen)) {
					// Get the woomen license data before unsetting
					$woomenLicense = $licenseItems->woomen;
					if (isset($woomenLicense->license) && $woomenLicense->license == 'valid') {
						update_option('woomen', $woomenLicense);
						update_option('woomen_key', $licenseKey);
						update_option('woomen-status', $woomenLicense->license);
					}

					// Now unset the woomen license data
					unset($body->licenses_data->{$licenseKey}->woomen);
				}
			}
		}


		// Remove "woomen" from addon_licenses
		if (isset($body->addon_licenses) && ! empty($body->addon_licenses)) {
			foreach ($body->addon_licenses as $paymentId => $addons) {
				if (isset($addons->woomen)) {
					unset($body->addon_licenses->{$paymentId}->woomen);
				}
			}
		}
				...
				foreach ($body->licenses_data as $license_key => $response) {
					foreach ($response as $slug => $data) {
						if ($data->license == 'valid') {
							...
							$addons[] = $slug;
							if ($slug == 'nextwp-sites') {
								update_option('edd_license_key', $license_key);
								update_option('edd_license_status', $data->license);
							}
						}
					}
				}
...
function cwp_activate_license_cb()
{
	if (isset($_POST['security_nonce']) && wp_verify_nonce($_POST['security_nonce'], 'cubewp-admin-nonce')) {
		...
		if (! file_exists($pluginDir)) {
			...
			if ($download_link) {
				...
				if ($result && ! is_wp_error($result)) {
					// Get plugin file path (e.g., plugin-folder/plugin-main-file.php)
					if (empty($base)) {
						$exceptions = ['valuepack-addons', 'nextwp-sites'];
						if (!in_array($slug, $exceptions)) {
					...
					$res = esc_html__('Failed to download file/No file founded!', 'woomen');
					wp_send_json($res);
				}
			} else {
				$res = esc_html__('Failed to download file/No file founded!', 'woomen');
			...
			if (isset($_POST['source']) && ! empty($_POST['source'])) {
				...
				$exceptions = ['valuepack-addons', 'nextwp-sites']; // Add any other slugs you want to skip
		...
		$res = ucwords(str_replace('-', ' ', $slug)) . esc_html__(' Activated!', 'woomen');
