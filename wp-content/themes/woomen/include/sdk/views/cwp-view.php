<?php
function cwp_license_manager_cb()
{
    include_once ABSPATH . 'wp-admin/includes/plugin.php';
    $cwp_license_nonce        = wp_create_nonce('cwp_license_form_nonce');
    $CWP_License_verification = new CWP_License_verification();
    $redirect_url             = $CWP_License_verification->setup_after_url;
    $selector                 = $CWP_License_verification->selector;
    $licensing                = $CWP_License_verification->licensing;
    $root                     = $CWP_License_verification->root;
    $PATH_URL                 = $CWP_License_verification->PATH_URL;
    $addons                   = get_option('associated-addons');
    $cwp_status = isset($_GET['cwp']) ? sanitize_text_field($_GET['cwp']) : '';
    $is_active  = ($cwp_status === 'success') ? 'active' : '';
?>

    <div class="woomen-theme-importer" data-redirect_url="<?php echo esc_url($redirect_url); ?>"
        data-selector="<?php echo esc_attr($selector); ?>">
        <div class="importer-container-fluids">
            <div class="importer-main-content">
                <div class="importer-left-content">
                    <div class="cube-setup-grid-progress">
                        <div class="cube-setup-grid-header">
                            <div class="main-title">
                                <h3><?php echo esc_html__('Setup Wizard', 'woomen'); ?></h3>
                                <p><?php echo esc_html__('Complete all the steps below.', 'woomen'); ?></p>
                            </div>
                        </div>
                        <div class="cube-setup-grid-list">
                            <ul class="cube-setup-list-theme">
                                <li class="progress-list active welcome">
                                    <div class="check-icons">
                                        <img class="hide-color"
                                            src="data:image/png;base64,">
                                        <img class="colored"
                                            src="data:image/png;base64,">
                                    </div>
                                    <?php echo esc_html__('Welcome', 'woomen'); ?>
                                </li>
                                <?php if (!get_option('woomen_import_completed_once')) { ?>
                                    <?php if ($licensing && empty($addons)) {
                                    ?>
                                        <li class="progress-list license <?php echo esc_attr($is_active); ?>">
                                            <div class="check-icons">
                                                <img class="hide-color"
                                                    src="data:image/png;base64,">
                                                <img class="colored"
                                                    src="data:image/png;base64,">
                                            </div>
                                            <?php echo esc_html__('License Verification', 'woomen'); ?>
                                        </li>
                                    <?php } ?>
                                    <li class="progress-list plugins <?php echo esc_attr($is_active); ?>">
                                        <div class="check-icons">
                                            <img class="hide-color"
                                                src="data:image/png;base64,">
                                            <img class="colored"
                                                src="data:image/png;base64,">
                                        </div>
                                        <?php echo esc_html__('Required Plugins', 'woomen'); ?>
                                    </li>
                                    <li class="progress-list content <?php echo esc_attr($is_active); ?>">
                                        <div class="check-icons">
                                            <img class="hide-color"
                                                src="data:image/png;base64,">
                                            <img class="colored"
                                                src="data:image/png;base64,">
                                        </div>
                                        <?php echo esc_html__('Dummy Content', 'woomen'); ?>
                                    </li>
                                    <li class="progress-list completed">
                                        <div class="check-icons">
                                            <img class="hide-color"
                                                src="data:image/png;base64,">
                                            <img class="colored"
                                                src="data:image/png;base64,">
                                        </div>
                                        <?php echo esc_html__('Setup Completed!', 'woomen'); ?>
                                    </li>
                                    <li class="progress-list failed hide">
                                        <div class="check-icons">
                                            <img
                                                src="data:image/png;base64,">
                                        </div>
                                        <?php echo esc_html__('Setup Failed!', 'woomen'); ?>
                                    </li>
                                <?php } else { ?>
                                    <li class="progress-list woomen-restart-importer">
                                        <div class="check-icons">
                                            <span class="dashicons dashicons-image-rotate"></span>
                                        </div>
                                        <?php echo esc_html__('Restart Importer', 'woomen'); ?>
                                    </li>
                                <?php } ?>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="importer-right-content">
                    <div class="cubewp-theme-importer-main">
                        <div class="loader-import-data">
                            <img src="<?php echo esc_url($PATH_URL . '/sdk/assets/images/importer/loader.gif'); ?>">
                        </div>
                        <div class="importer-tab <?php if (! isset($_GET['cwp'])) {
                                                        echo 'active';
                                                    } ?>">
                            <!---* Welcome page screen *-->
                            <?php include $root . '/views/cwp-setup-welcome.php'; ?>
                        </div>
                        <?php if ($licensing && empty($addons)) { ?>
                            <div class="importer-tab licensing">
                                <div class="importer-step-form-lisance">
                                    <?php include $root . '/views/cwp-setup-header.php'; ?>
                                    <div class="importer-step-form-lisance-main">
                                        <img
                                            src="<?php echo esc_url($PATH_URL . '/sdk/assets/images/importer/step_2_search_icon.png'); ?>">
                                        <h2 class="License-Verification-title"><?php echo esc_html__('License Verification', 'woomen'); ?></h2>
                                        <p class="License-Verification-des"><?php echo esc_html__('Please verify your purchase by entering the license key available in your account.', 'woomen'); ?></p>
                                        <form method="post" class="verify-license" id="cwp_license_verification">
                                            <input type="hidden" name="cwp_license_form_meta_nonce"
                                                value="<?php echo esc_attr($cwp_license_nonce); ?>" />
                                            <div class="input-key-theme">
                                                <label
                                                    for="cwp_license_key"> <?php echo esc_html__('Enter Your Purchase Key', 'woomen'); ?> </label>
                                                <input required class="form-input-install" id="cwp_license_key"
                                                    type="text" name="cwp_license_key" value=""
                                                    placeholder="<?php esc_attr_e('Put your Envato product license key here..', 'woomen'); ?>" />
                                            </div>
                                            <div class="input-key-theme">
                                                <label
                                                    for="cwp_user_email"> <?php echo esc_html__('Email Address (If you dont want to use admin email)', 'woomen'); ?> </label>
                                                <input id="cwp_user_email" type="text" name="cwp_user_email"
                                                    class="form-input-install" value=""
                                                    placeholder="<?php esc_attr_e('This email will be used for managing CubeWP account on cubewp.com', 'woomen'); ?>" />
                                            </div>
                                            <div class="input-key-theme">
                                                <input type="submit" id="cwp_license_submit" name="submit"
                                                    value="<?php echo esc_attr__('Verify License', 'woomen'); ?>">
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="importer-tab plugins">
                            <div class="importer-step-form-lisance">
                                <?php include $root . '/views/cwp-setup-header.php'; ?>
                                <div class="importer-step-form-lisance-main">
                                    <img class="plugin-rocket"
                                        src="<?php echo esc_url($PATH_URL . '/sdk/assets/images/importer/step_3_rocket_icon.png'); ?>">
                                    <h2 class="plugin-required-title"><?php echo esc_html__('Required Plugins Installation', 'woomen'); ?></h2>
                                    <p class="plugin-required-des"><?php echo esc_html__('These plugins are required to import dummy content', 'woomen'); ?></p>
                                    <div class="verify-license">
                                        <div class="cube-setup-grid-list">
                                            <ul class="cube-setup-list-theme">
                                                <?php
                                                if (empty($addons)) {
                                                    $addons = array();
                                                }
                                                if (function_exists('cwp_theme_required_plugins')) {
                                                    $theme_required = cwp_theme_required_plugins();
                                                    if (! empty($theme_required) && is_array($theme_required)) {
                                                        foreach ($theme_required as $theme_req) {
                                                            if (isset($theme_req['slug']) && isset($theme_req['base']) && isset($theme_req['name'])) {
                                                                $theme_req['cwp-source'] = $theme_req['cwp-source'] ?? 'no';
                                                                if (! in_array($theme_req['slug'], $addons) && $theme_req['cwp-source'] != 'yes') {
                                                ?>
                                                                    <li class="progress-list active"
                                                                        id="parent-<?php echo esc_attr($theme_req['slug']); ?>">
                                                                        <div class="check-icons">
                                                                            <?php
                                                                            if (isset($theme_req['class_exists']) && ! class_exists($theme_req['class_exists'])) { ?>
                                                                                <input type="checkbox"
                                                                                    id="<?php echo esc_attr($theme_req['slug']); ?>"
                                                                                    name="cwp-plugins-installation"
                                                                                    data-base="<?php echo esc_attr($theme_req['base']); ?>"
                                                                                    data-source="<?php echo isset($theme_req['source']) ? esc_attr($theme_req['source']) : ''; ?>"
                                                                                    value="<?php echo esc_attr($theme_req['slug']); ?>"
                                                                                    name="cwp-plugins-installation"
                                                                                    checked <?php if (isset($theme_req['required']) && $theme_req['required'] == 'yes') {
                                                                                                echo 'disabled';
                                                                                            } ?>>
                                                                            <?php } else { ?>
                                                                                <img class="colored"
                                                                                    src="data:image/png;base64,">
                                                                            <?php } ?>
                                                                        </div>
                                                                        <?php
                                                                        echo esc_html($theme_req['name']);
                                                                        ?>
                                                                    </li>
                                                        <?php
                                                                }
                                                            }
                                                        }
                                                    }
                                                }
                                                if (isset($addons)) {
                                                    foreach ($addons as $addon) {
                                                        ?>
                                                        <li class="progress-list active"
                                                            id="parent-<?php echo esc_attr($addon); ?>">
                                                            <div class="check-icons">
                                                                <?php
                                                                $addon_data  = get_option($addon);
                                                                $license_key = get_option($addon . '_key');
                                                                $pluginDir   = WP_PLUGIN_DIR . '/' . $addon;
                                                                $exceptions = ['valuepack-addons']; // Add any other slugs you want to skip

                                                                if (in_array($addon, $exceptions)) {
                                                                    $base = $addon; // Keep original
                                                                } else {
                                                                    $base = str_replace('-addon', '', str_replace('-pro', '', $addon));
                                                                }
                                                                $file_name = str_replace('-', ' ', $base);
                                                                $file_name = str_replace('cubewp', 'CubeWp', $file_name);
                                                                $base      = str_replace(' ', '_', ucwords($file_name));
                                                                if ($addon == 'cubewp-framework') {
                                                                    $base = 'CubeWp';
                                                                }
                                                                if (! class_exists($base . '_Load')) { ?>
                                                                    <?php if (isset($addon_data) && ! empty($addon_data)) { ?>
                                                                        <input type="checkbox"
                                                                            id="<?php echo esc_attr($addon); ?>"
                                                                            value="<?php echo esc_attr($addon); ?>"
                                                                            data-order="<?php echo esc_attr($addon_data->payment_id); ?>"
                                                                            data-license="<?php echo esc_attr($license_key); ?>"
                                                                            data-download_id="<?php echo esc_attr($addon_data->item_id); ?>"
                                                                            data-cwp_source="yes"
                                                                            name="cwp-plugins-installation" checked>
                                                                    <?php } else { ?>
                                                                        <input type="checkbox"
                                                                            id="<?php echo esc_attr($addon); ?>"
                                                                            value="<?php echo esc_attr($addon); ?>"
                                                                            name="cwp-plugins-installation" checked>
                                                                    <?php } ?>
                                                                <?php } else { ?>
                                                                    <img class="colored"
                                                                        src="data:image/png;base64,">
                                                                <?php } ?>
                                                            </div>
                                                            <?php
                                                            $plugin_name = str_replace('-addon', ' ', str_replace('-pro', ' ', $addon));
                                                            $plugin_name = ucwords(str_replace('-', ' ', $plugin_name));
                                                            $plugin_name = str_replace('Cubewp', 'CubeWP', $plugin_name);
                                                            echo esc_html($plugin_name);
                                                            ?>
                                                        </li>
                                                <?php }
                                                } ?>
                                            </ul>
                                        </div>
                                        <h4 class="verify-title"></h4>
                                        <p class="verify-name"></p>
                                    </div>
                                    <div id="install-plugins"
                                        data-cubeframework="<?php if (class_exists('CubeWp_Load')) {
                                                                echo esc_attr('enabled');
                                                            } ?>"><?php if (empty($addons)) { ?><?php echo esc_html__('Install Plugins', 'woomen'); ?><?php } else { ?><?php echo esc_html__('Continue to Import Process', 'woomen'); ?><?php } ?></div>
                                </div>
                            </div>
                        </div>
                        <div
                            class="importer-tab dummy_content <?php echo esc_attr( $is_active ); ?>">
                            <div class="importer-step-form-lisance">
                                <?php
                                include $root . '/views/cwp-setup-header.php';
                                if (! isset($_GET['demo'])) {
                                    $path     = $root . '/import/';
                                    $path_url = $PATH_URL . '/sdk/import/';
                                    $demos = cwp_get_theme_demo_styles();
                                    if (! empty($demos)) {
                                ?>
                                        <ul class="demo-type-styles">
                                            <?php
                                            foreach ($demos as $id => $demo) {
                                                $folder     = $demo['folder'] ?? '';
                                                $screenshot = $demo['screenshot'] ?? '';
                                                $preview    = $demo['preview'] ?? '';
                                                $demo_name  = $demo['demo_name'] ?? '';
                                                $demo_id    = $demo['demo_id'] ?? '';
                                                $url = 'admin.php?page=nextwp-sites&id=' . $demo_id . '&step=2';
                                                $demo_url   = admin_url($url);
                                            ?>
                                                <li class="importer-demo-style">
                                                    <div class="importer-demo-img">
                                                        <img src="<?php echo esc_url($path_url . $folder . '/' . $screenshot); ?>">
                                                    </div>
                                                    <div class="importer-demo-details">
                                                        <h6><?php echo esc_html($demo_name); ?></h6>
                                                        <div class="importer-demo-actions">
                                                            <a href="<?php echo esc_url($demo_url); ?>">
                                                                <?php esc_html_e('Import Site', 'woomen'); ?>
                                                            </a>
                                                            <a href="<?php echo esc_url($preview); ?>" target="_blank">
                                                                <span class="dashicons dashicons-external"></span>
                                                                <?php esc_html_e('Preview', 'woomen'); ?>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </li>
                                            <?php
                                            }
                                            ?>
                                        </ul>
                                <?php
                                    }
                                }
                                ?>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php
}
